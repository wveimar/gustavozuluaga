...
const PageSection = ({ sectionCode, code, children }) => {
  ...
  return (
    <section  style={{ textAlign: "center", marginTop: "4rem" }} className="section-container">
      {pageSection && !pageSection.hideTitle && <h1 style={{textAlign:"center"}}>{pageSection.title}</h1>}
      {pageSection && !pageSection.hideDescription &&   <p style={{ marginBottom: "4rem" }}>{pageSection.description}</p>}
      <div>{children}</div>
    </section>
  );
};
